...
function customAssign(...objects) {
  ...
  if (objects.length < 2) {
    ...
    for (let elements in newObject) {
      if (typeof newObject[elements] === "number") {
        newObject[elements] = newObject[elements].toString();
      }
    }
    return newObject;
  }
...
function customObjectFromEntries(entries) {
  ...
  if (!Array.isArray(entries)) {
    ...
    let check = entries.map(([key, value]) => {
    ...
    let result = Object.fromEntries(check);
    return result;
  }
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  let assignObject = Object.assign(obj1, obj2);
  let freezeObject = Object.freeze(assignObject);

  if (Object.isFrozen(freezeObject)) {
    return freezeObject;
  } else {
...
function filterObjectsByKey(arr, key) {
  ...
  let fliterArray = arr.filter((obj) => obj[key])
  return fliterArray;
}
...
function checkAndPreventExtensions(obj) {
  ...
  let check = Object.isExtensible(obj);
...
function sealAndCheck(obj) {
  ...
  if (typeof obj !== "object") {
    ...
    let closeObject = Object.seal(obj);
...
function getObjectValuesSum(obj) {
  ...
  if (typeof obj !== "object") {
    ...
    let sum = getValues.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
    return sum;
  }
}
